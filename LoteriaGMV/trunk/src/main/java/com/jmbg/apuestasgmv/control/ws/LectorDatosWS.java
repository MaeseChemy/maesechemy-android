package com.jmbg.apuestasgmv.control.ws;
...
public class LectorDatosWS {
	...
	private Bet generateBet(String line) {
		...
		try {
			...
			if (mHttpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				if (entity != null) {
					// insert to database
					bet.setBetImage(EntityUtils.toByteArray(entity));
				}
			}
		} catch (ClientProtocolException e) {
			e.printStackTrace();
	...
	private String convertPriceDate(String date) {
		...
		// 20/01/2014
		String convertDate = getPriceDay(campos[1]) + "/"
				+ getPriceMonth(campos[3]) + "/" + campos[5];
